...
### Added
- **Channel-mode + per-user sessions (`mode: 'thread' | 'channel'` per platform).** New `mode` field on `PlatformInstanceConfig` opts a deployment into a non-threaded session model. When `mode: channel` is set, the bot replies as channel root posts (no thread reply) and each user @mentioning the bot gets their own concurrent session — multiple users in the same channel = multiple parallel Claude instances. Sessions are keyed by `platformId:channelId:userId` instead of `platformId:threadId`. The full wire-up lands in this PR: config flag + validation, `Session.mode`/`userId`/`channelId` fields, `PersistedSession` round-trip with backward-compat defaults, per-platform mode tracking in `SessionManager.platformMode`, three-part `getSessionId`, the conditional `replyTo` in `createPostAndTrack`, `SessionRegistry.findByChannelUser` (mode-scoped so it never crosses with thread-mode sessions), the message-handler routing branch that swaps `findByThreadId` for `findChannelSession` when the platform is in channel mode, the `initialOptions.channelMode` plumbing through `lifecycle.startSession` so new channel-mode sessions are constructed with the right identity, and a `directReplyTo` for the handful of direct `createPost` calls in `message-handler.ts` (kill confirmations, error notifications, "Mention me with your request") so they land at the channel root rather than as stray thread replies. Default mode is `'thread'` everywhere; existing deployments are completely unaffected.
